	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				...
				let url = `http://localhost:8080/api/debitPaid`;

				// Determine the correct endpoint based on custId and year
				if (custId && year) {
					url += `/filter/${custId}/${year}`;
				} else if (custId) {
					url += `/custId/${custId}`;
				} else if (year) {
					url += `/year/${year}`;
				...
				// Assuming your API returns the total number of pages in a 'totalPages' field
